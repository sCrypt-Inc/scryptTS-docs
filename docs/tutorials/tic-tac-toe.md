...
A public `@method` can be called from an external transaction. The call succeeds if it runs to completion without violating any conditions in `assert()`. Function `assert(condition: boolean, errorMsg?: string)` throws an error if the `condition` is false; otherwise it does nothing. 
...
### Array
`sCrypt` only allows fixed-size array. When you declare an array, you must use `FixedArray<T, N>`. `T` is the type of array element and `N` is the array size. For the Tic-Tac-Toe game, a player wins if three of his marks are in a straight line. We enumerate all possibilities in a two-dimensional array `FixedArray<FixedArray<bigint, 3>, 8>`.
