...
# Tutorial 5: How to debug ScriptContext error
...
A function `diffOutputs` is provided to directly compare the difference between `outputs` in the contract and the outputs of the transaction, and print out the result.

Just call `this.diffOutputs(outputs)` in the contract:

```ts
this.diffOutputs(outputs); // diff and print the comparison result
assert(this.ctx.hashOutputs == hash256(outputs));
```
