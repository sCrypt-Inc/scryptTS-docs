...
```ts
let output = this.buildStateOutput();
```
...
if (this.won(play)) {
  ...
  const outputScript: ByteString = TxUtils.buildP2PKHScript(
    pubKey2Addr(player)
  );
  outputs: ByteString = TxUtils.buildOutput(outputScript, this.ctx.spentAmount);
} else if (this.full()) {
  const halfAmount = this.ctx.spentAmount / 2n;
  const aliceOutput: ByteString = TxUtils.buildP2PKHOutput(
    this.alice,
    TxUtils.toSatoshis(halfAmount)
  );
  const bobOutput: ByteString = TxUtils.buildP2PKHOutput(
    this.bob,
    TxUtils.toSatoshis(halfAmount)
  );
...
export class TicTacToe extends SmartContract {
  ...
  public move(n: bigint, sig: Sig) {
    ...
    if (this.won(play)) {
      ...
      outputs: ByteString = TxUtils.buildOutput(
        outputScript,
